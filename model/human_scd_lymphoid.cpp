...
Comparison between the scd model and the erythriod_lymphoid_meyloid model: 
introducing sickle-beta and HbS related dynamics
...
Kdalphabeta = 1e-2 // αβˢ dimer dissociation constant, nM; 
...
ratiosynbetanew = 0.5 // INTRODUCED FOR EX VIVO EDITED HSC
Kdalphabetanew = 1e-3 // αβ^T87Q dimer dissociation constant, nM; this is for sickle gene
...
double koffalphabetanew = Kdalphabetanew * kon;
...
double vO2 = ( HbS_saturation * HbS + HbA_saturation * HbA + HbF_saturation * HbF + HbA2_saturation * HbA2 ) * 1.34;
...
dxdt_alphabeta_RET0  = kon * alpha_RET0 * beta_RET0   - kRET2RBC * alphabeta_RET0  - koffalphabeta * alphabeta_RET0  * VRET0 - 2 * kon * alphabeta_RET0  * alphabeta_RET0  + 2 * koffHbA  *VRET0 * HbS_RET0 ;
...
dxdt_HbS_RET0  = kon * alphabeta_RET0  * alphabeta_RET0  - (koffHbA  *VRET0 + kRET2RBC) * HbS_RET0;
...
dxdt_alphabeta_RBC0  = kRET2RBC * alphabeta_RET0  - kdeathRBCendo * alphabeta_RBC0  + kon * alpha_RBC0 * beta_RBC0  - koffalphabeta *VRBC0 * alphabeta_RBC0  - 2*kon* alphabeta_RBC0 *alphabeta_RBC0  + 2*koffHbS *VRBC0 * HbS_RBC0;
...
dxdt_alpha_RET1   = ksynalpha                      * RET1 - (kdeg + kRET2RBC)* alpha_RET1   - (kon*alpha_RET1*beta_RET1 + kon*alpha_RET1*betanew_RET1 + kon*alpha_RET1*gamma_RET1 + kon*alpha_RET1*delta_RET1) + (koffalphabeta*alphabeta_RET1 + koffalphabetanew*alphabetanew_RET1 + koffalphagamma*alphagamma_RET1 + koffalphadelta*alphadelta_RET1)*VRET1;
dxdt_beta_RET1    = ksynbeta * (1-ratiosynbetanew) * RET1 - (kdeg + kRET2RBC)* beta_RET1    - kon*alpha_RET1*beta_RET1    + koffalphabeta *VRET1 * alphabeta_RET1; 
dxdt_betanew_RET1 = ksynbeta * ratiosynbetanew     * RET1 - (kdeg + kRET2RBC)* betanew_RET1 - kon*alpha_RET1*betanew_RET1 + koffalphabetanew  *VRET1 * alphabetanew_RET1;
...
dxdt_alphabeta_RET1    = kon * alpha_RET1 * beta_RET1    - koffalphabeta *VRET1* alphabeta_RET1    - 2*kon* alphabeta_RET1    * alphabeta_RET1    + 2*koffHbS *VRET1* HbS_RET1  - kRET2RBC * alphabeta_RET1;
dxdt_alphabetanew_RET1 = kon * alpha_RET1 * betanew_RET1 - koffalphabetanew  *VRET1* alphabetanew_RET1 - 2*kon* alphabetanew_RET1 * alphabetanew_RET1 + 2*koffHbA *VRET1* HbA_RET1  - kRET2RBC * alphabetanew_RET1;
...
dxdt_betanew_RBC1 = kRET2RBC * betanew_RET1 - (kdeg + kdeathRBCtrans) * betanew_RBC1 - kon * alpha_RBC1 * betanew_RBC1 + koffalphabetanew *VRBC1* alphabetanew_RBC1;
...
dxdt_alphabetanew_RBC1 = kRET2RBC * alphabetanew_RET1 - kdeathRBCtrans * alphabetanew_RBC1 + kon * alpha_RBC1 * betanew_RBC1 - koffalphabetanew  *VRBC1* alphabetanew_RBC1 - 2*kon* alphabetanew_RBC1 *alphabetanew_RBC1 + 2*koffHbA *VRBC1* HbA_RBC1;
dxdt_alphabeta_RBC1    = kRET2RBC * alphabeta_RET1    - kdeathRBCtrans * alphabeta_RBC1    + kon * alpha_RBC1 * beta_RBC1  - koffalphabeta *VRBC1 * alphabeta_RBC1  - 2*kon* alphabeta_RBC1 *alphabeta_RBC1  + 2*koffHbS *VRBC1 * HbS_RBC1;
...
dxdt_cd4rec0 = sum_SP40 - death_naiveT_cd4 * cd4rec0 + exit_lymph * cd4lym0 - enter_lymph * cd4rec0 + exit_peripheral * cd4peripheral0 - enter_peripheral * cd4rec0 + k_nT_pro * (cd4rec0/bloodvolume)/(cd4rec0/bloodvolume + env_naiveT);
dxdt_cd8rec0 = sum_SP80 - death_naiveT_cd8 * cd8rec0 + exit_lymph * cd8lym0 - enter_lymph * cd8rec0 + exit_peripheral * cd8peripheral0 - enter_peripheral * cd8rec0 + k_nT_pro * (cd8rec0/bloodvolume)/(cd8rec0/bloodvolume + env_naiveT);

dxdt_cd4rec1 = sum_SP41 - death_naiveT_cd4 * cd4rec1 + exit_lymph * cd4lym1 - enter_lymph * cd4rec1 + exit_peripheral * cd4peripheral1 - enter_peripheral * cd4rec1 + k_nT_pro * (cd4rec1/bloodvolume)/(cd4rec1/bloodvolume + env_naiveT);
dxdt_cd8rec1 = sum_SP81 - death_naiveT_cd8 * cd8rec1 + exit_lymph * cd8lym1 - enter_lymph * cd8rec1 + exit_peripheral * cd8peripheral1 - enter_peripheral * cd8rec1 + k_nT_pro * (cd8rec1/bloodvolume)/(cd8rec1/bloodvolume + env_naiveT);

// dynamics of T cells in peripheral tissues
dxdt_cd4peripheral0 = k_nT_pro * (cd4peripheral0/peripheralvolume)/(cd4peripheral0/peripheralvolume + env_naiveT) - death_naiveT_cd4 * cd4peripheral0 - exit_peripheral * cd4peripheral0 + enter_peripheral * cd4rec0;
dxdt_cd8peripheral0 = k_nT_pro * (cd8peripheral0/peripheralvolume)/(cd8peripheral0/peripheralvolume + env_naiveT) - death_naiveT_cd8 * cd8peripheral0 - exit_peripheral * cd8peripheral0 + enter_peripheral * cd8rec0;

dxdt_cd4peripheral1 = k_nT_pro * (cd4peripheral1/peripheralvolume)/(cd4peripheral1/peripheralvolume + env_naiveT) - death_naiveT_cd4 * cd4peripheral1 - exit_peripheral * cd4peripheral1 + enter_peripheral * cd4rec1;
dxdt_cd8peripheral1 = k_nT_pro * (cd8peripheral1/peripheralvolume)/(cd8peripheral1/peripheralvolume + env_naiveT) - death_naiveT_cd8 * cd8peripheral1 - exit_peripheral * cd8peripheral1 + enter_peripheral * cd8rec1;
...
capture totalHb = HbS + HbA + HbF + HbA2; // unit: g/dL
capture HbinRBC = (HbS_RBC0 + HbS_RBC1 + HbA_RBC1 + HbF_RBC0 + HbF_RBC1 + HbA2_RBC0 + HbA2_RBC1) * MWHh*1e-9 / (VRBC0 + VRBC1); // unit: g/L
...
[CAPTURE]
aCFUE, sum_SP40, sum_SP80, sum_SP41, sum_SP81, HbS, HbA, HbF, HbA2
